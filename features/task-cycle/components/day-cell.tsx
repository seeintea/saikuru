...
export default function DayCell({ day, onPress, size = "medium" }: DayCellProps) {
  ...
  const CellContent = () => (
    ...
    >
      <Text style={[styles.dayName, { fontSize: getDayNameSize() }, isTodayDate && styles.todayText]}>
        {getDayName(day.date)}
      </Text>
      <Text style={[styles.dayNumber, { fontSize: getDayNumberSize() }, isTodayDate && styles.todayText]}>
        {day.date.getDate()}
      </Text>
